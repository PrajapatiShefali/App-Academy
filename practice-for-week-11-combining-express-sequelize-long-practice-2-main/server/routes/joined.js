...
router.get('/trees-insects', async (req, res, next) => {
    ...
    trouter.get('/trees-insects', async (req, res) => {
        try {
          const trees = await Trees.findAll({
            include: {
              ...
              order: [['name', 'ASC']] // Order insects alphabetically by name
            },
            attributes: ['id', 'tree', 'location', 'heightFt'], // Exclude updatedAt and createdAt from the response
            having: Sequelize.literal('COUNT(Insects.id) > 0'), // Only include trees with insects
            group: ['Trees.id']
          });
