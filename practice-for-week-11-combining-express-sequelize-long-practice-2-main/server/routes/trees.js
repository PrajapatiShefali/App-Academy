...
router.get('/', async (req, res, next) => {
    let trees = [];
    // Your code here
    trees=await models.Trees.findAll({
...
router.get('/:id', async (req, res, next) => {
    ...
    try {
        // Your code here
        tree=await models.Trees.findOne({    
            where:{
                id:req.params.id
            }
        })
...
router.post('/', async (req, res, next) => {
    let body=await models.Trees.create({
        // name:'Lemon Tree',
        // location:'National Park',
        // heightFt:123.2,
        // groundCircumferenceFt:143.2,
        id:25 , 
        tree:req.body.name ,
    ...
    let getnewTree=await models.Trees.findOne({where:{tree:req.body.name}})
    try {
        res.json({
            status: "success",
...
router.delete('/:id', async (req, res, next) => {
    const deleteTree=await models.Trees.destroy( {where:{id:req.params.id}})
    try {
        res.json({
            status: "success",
...
router.put('/:id', async (req, res, next) => {
    try {
        // Your code here
        let getTree=await models.Trees.findOne({where:{id:req.params.id}})
         getTree.set({
            tree:req.body.name,
            location:req.body.location,
            heightFt:req.body.height,
